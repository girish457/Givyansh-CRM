...
const assignGamificationPoints = async (candidate, isNew) => {
  const logHook = (msg) => {
    try {
      fs.appendFileSync("c:\\Users\\goswa\\OneDrive\\Desktop\\Fast RMS\\Fast RMS\\gamification_hook_log.txt", `[${new Date().toISOString()}] Candidate ${candidate.id || 'new'} | isNew: ${isNew} | ${msg}\n`);
    } catch (e) {}
  };
