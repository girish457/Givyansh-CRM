...
// Temporary search script to find MeetingAttendance references
try {
  const fs = require('fs');
  const path = require('path');
  function searchDir(dir, pattern, results = []) {
    if (dir.includes('node_modules') || dir.includes('.git') || dir.includes('.gemini') || dir.includes('dist')) {
      return results;
    }
  ...
  const results = searchDir(workspaceRoot, 'MeetingAttendance');
  fs.writeFileSync(path.resolve(__dirname, '../search_results.txt'), results.join('\n'));
} catch (e) {
  try {
    fs.writeFileSync(path.resolve(__dirname, '../search_results.txt'), "Error: " + e.message + "\n" + e.stack);
  } catch (err) {}
}
...
app.use((req, res, next) => {
    ...
    res.json = function(body) {
        if (res.is500) {
            try {
                fs.appendFileSync(path.resolve(__dirname, "../server_error.log"), `[${new Date().toISOString()}] Path: ${req.path} | Error: ${JSON.stringify(body)}\n`);
            } catch(e) {}
        }
...
app.use((err, req, res, next) => {
    if (err) {
        try {
            fs.writeFileSync(path.resolve(__dirname, "../express_error.txt"), "Express Error: " + err.message + "\nStack: " + err.stack + "\nPath: " + req.path);
        } catch (writeErr) {
...
connectDB().then(async () => {
        ...
        try {
            const migrations = [
                ...
                `CREATE TABLE IF NOT EXISTS webrtc_signals (
                    id INT AUTO_INCREMENT PRIMARY KEY,
                    meetingId VARCHAR(64) NOT NULL,
                    fromUserId INT NOT NULL,
                    toUserId INT NOT NULL,
                    data LONGTEXT NOT NULL,
                    createdAt DATETIME DEFAULT CURRENT_TIMESTAMP,
                    INDEX idx_signal_to (meetingId, toUserId),
                    INDEX idx_signal_created (createdAt)
                )`,
                "ALTER TABLE notifications MODIFY COLUMN type VARCHAR(255) NOT NULL",
                ...
                "ALTER TABLE Shifts ADD COLUMN startDate DATE NULL",
                "ALTER TABLE Shifts ADD COLUMN endDate DATE NULL",
                "ALTER TABLE shifts ADD COLUMN earlyLoginAllowed INT NOT NULL DEFAULT 60",
                "ALTER TABLE shifts ADD COLUMN postShiftAllowed INT NOT NULL DEFAULT 120",
                "ALTER TABLE shifts ADD COLUMN startDate DATE NULL",
                "ALTER TABLE shifts ADD COLUMN endDate DATE NULL",
                ...
                "ALTER TABLE Users ADD COLUMN lastReadMessageId INT DEFAULT 0",
                "ALTER TABLE meeting_attendances ADD COLUMN cameraActive BOOLEAN DEFAULT FALSE",
                "ALTER TABLE meeting_attendances ADD COLUMN micActive BOOLEAN DEFAULT FALSE",
                "ALTER TABLE meeting_attendances ADD COLUMN screenshareActive BOOLEAN DEFAULT FALSE",
                "ALTER TABLE meeting_attendances ADD COLUMN lastHeartbeat DATETIME NULL",
                "ALTER TABLE meeting_attendances ADD COLUMN handRaised BOOLEAN DEFAULT FALSE",
                "ALTER TABLE meeting_attendances ADD COLUMN mutedByHost BOOLEAN DEFAULT FALSE",
                "ALTER TABLE meeting_attendances ADD COLUMN videoDisabledByHost BOOLEAN DEFAULT FALSE",
                "ALTER TABLE MeetingAttendances ADD COLUMN cameraActive BOOLEAN DEFAULT FALSE",
                "ALTER TABLE MeetingAttendances ADD COLUMN micActive BOOLEAN DEFAULT FALSE",
                "ALTER TABLE MeetingAttendances ADD COLUMN screenshareActive BOOLEAN DEFAULT FALSE",
                "ALTER TABLE MeetingAttendances ADD COLUMN lastHeartbeat DATETIME NULL",
                "ALTER TABLE MeetingAttendances ADD COLUMN handRaised BOOLEAN DEFAULT FALSE",
                "ALTER TABLE MeetingAttendances ADD COLUMN mutedByHost BOOLEAN DEFAULT FALSE",
                "ALTER TABLE MeetingAttendances ADD COLUMN videoDisabledByHost BOOLEAN DEFAULT FALSE",
                "ALTER TABLE meetings ADD COLUMN sharedModule VARCHAR(255) NULL",
                "ALTER TABLE meetings ADD COLUMN presenterId INT NULL",
                "ALTER TABLE Meetings ADD COLUMN sharedModule VARCHAR(255) NULL",
                "ALTER TABLE Meetings ADD COLUMN presenterId INT NULL"
            ];
    ...
    if (!isDbConnected) {
        ...
        sequelize.sync()
            .then(async () => {
                try { fs.writeFileSync(path.resolve(__dirname, "../test_full_diagnostics.txt"), "SERVER_IS_RUNNING"); } catch (e) {}
                ...
                try {
                    ...
                    try { fs.writeFileSync(path.resolve(__dirname, "../test_full_diagnostics.txt"), "ERROR: " + testErr.stack); } catch (e) {}
                }
                ...
                try {
                    ...
                    if (testUser) {
                        ...
                        try { fs.writeFileSync(path.resolve(__dirname, "../test_tasks_query.txt"), "SUCCESS: Found " + allTasks.length + " tasks"); } catch (e) {}
                    } else {
                        try { fs.writeFileSync(path.resolve(__dirname, "../test_tasks_query.txt"), "WARNING: No user found for diagnostic"); } catch (e) {}
                    }
                } catch (queryErr) {
                    try { fs.writeFileSync(path.resolve(__dirname, "../test_tasks_query.txt"), "ERROR: " + queryErr.stack); } catch (e) {}
                }
...
// Refresh point - trigger reload 1
// Start Node
app.get("/", (req, res) => {
...
const validateShiftWindow = (shift) => {
    ...
    const { dateStr, hours, minutes } = getKolkataDateTime();

    // 1. Validate Active Dates
    if (shift.startDate && dateStr < shift.startDate) {
        return {
            allowed: false,
            message: `Your shift has not started yet. It will be active from ${shift.startDate}.`
        };
    }
    if (shift.endDate && dateStr > shift.endDate) {
        return {
            allowed: false,
            message: `Your shift access has expired. It was valid till ${shift.endDate}.`
        };
    }

    // 2. Validate Daily Access Window
...
app.get("/api/me", async (req, res) => {
    ...
    try {
        ...
        const user = await User.findByPk(decoded.userId, {
            attributes: ["id", "name", "email", "role", "reportingTo", "shiftId", "companyId"],
            include: [
                {
                    model: User,
                    as: "manager_tl",
...
app.post("/api/tasks", authenticate, async (req, res) => {
    try {
        ...
        try {
            fs.writeFileSync(path.resolve(__dirname, "../error_log.txt"), err.stack || err.message);
        } catch (writeErr) {
...
// --- Duplicate Vendor Management Endpoints removed (they override the correct comprehensive ones defined around line 11752) ---
// Restart trigger: 2026-06-18T21:38:00
...
app.get("/api/meetings/company-settings", authenticate, async (req, res) => {
    try {
...
// Update company settings (Boss only)
app.put("/api/meetings/company-settings", authenticate, async (req, res) => {
    try {
        const { companyId, role } = req.user;
        if (role !== "boss") {
            return res.status(403).json({ error: "Only the Boss can configure meeting controls." });
        }
        const {
            moduleEnabled, maxDuration, recordingAllowed, screenshareAllowed,
            chatAllowed, cameraAllowed, micAllowed, aiSummaryAllowed, reminderTimings
        } = req.body;

        await MeetingSetting.upsert({
            companyId, moduleEnabled, maxDuration, recordingAllowed, screenshareAllowed,
            chatAllowed, cameraAllowed, micAllowed, aiSummaryAllowed, reminderTimings
        });

        res.json({ success: true, message: "Meeting settings updated successfully." });
    } catch (err) {
        res.status(500).json({ error: err.message });
    }
});
...
app.get("/api/meetings/participants-directory", authenticate, async (req, res) => {
    try {
        const { companyId } = req.user;
        const users = await User.findAll({
            where: { companyId, status: "active", role: { [Op.ne]: "superadmin" } },
...
// Create new meeting (scheduled or instant)
app.post("/api/meetings", authenticate, async (req, res) => {
    try {
        const { userId, companyId, name, role } = req.user;
        if (role === "recruiter") {
            return res.status(403).json({ error: "Recruiters cannot schedule meetings." });
        }

        const {
            title, agenda, description, meetingType,
            scheduledDate, scheduledTime, duration, priority, participants
        } = req.body;

        // Check company settings
        const settings = await MeetingSetting.findByPk(companyId);
        if (settings && !settings.moduleEnabled) {
            return res.status(403).json({ error: "The meeting module is disabled by the Boss." });
        }

        const participantsArray = Array.isArray(participants) ? participants : [];
        // Ensure host is not in participants list, but is tracked separately
        const cleanParticipants = participantsArray.filter(pId => Number(pId) !== Number(userId));
        ...
        const meeting = await Meeting.create({
            ...
            scheduledDate: meetingType === "instant" ? new Date().toISOString().split("T")[0] : scheduledDate,
            scheduledTime: meetingType === "instant" ? new Date().toTimeString().split(" ")[0].slice(0, 5) : scheduledTime,
        ...
        // If scheduled, send standard notifications
        if (meetingType !== "instant") {
            for (const pId of cleanParticipants) {
                await Notification.create({
                    userId: pId,
                    type: "system_alert",
                    title: "New Meeting Invitation",
                    message: `${name} (${role.toUpperCase()}) has invited you to a scheduled meeting: ${title} on ${scheduledDate} at ${scheduledTime}.`,
                    createdAt: new Date()
                });
            }
        } else {
            // Instant Meeting: Auto join the host
            await MeetingAttendance.create({
                meetingId: meeting.id,
                userId,
                joinTime: new Date(),
                status: "completed" // host is marked completed initially or live
            });

            // Send notification alerts to all participants
            for (const pId of cleanParticipants) {
                await Notification.create({
                    userId: pId,
                    type: "system_alert",
                    title: `Live Instant Meet: ${title}`,
                    message: `${name} has started an Instant Live Meeting right now! Click to Join.`,
                    createdAt: new Date()
                });
            }
        }
...
app.get("/api/meetings", authenticate, async (req, res) => {
    try {
        ...
        if (role === "recruiter") {
            ...
            meetings = allCompanyMeetings.filter(m => {
                try {
                    const pts = JSON.parse(m.participants || "[]");
                    return pts.map(Number).includes(Number(userId));
                } catch (e) {
                    return false;
                }
            });
...
// Get active notifications & alerts (Instant Meetings and upcoming reminders)
app.get("/api/meetings/active-alerts", authenticate, async (req, res) => {
    try {
        const { userId, companyId } = req.user;

        // 1. Fetch live instant meetings in the company that the user is invited to
        // and hasn't joined/rejected yet
        ...
        for (const m of liveMeetings) {
            try {
                const pts = JSON.parse(m.participants || "[]");
                if (pts.map(Number).includes(Number(userId)) || Number(m.hostId) === Number(userId)) {
                    // Check if already has an attendance record
                    const att = await MeetingAttendance.findOne({
                        where: { meetingId: m.id, userId }
                    });
                    if (!att || (att.status === "absent" && !att.rejected && !att.joinTime)) {
                        activeInstant.push(m);
                    }
                }
            } catch (e) {}
        }
        ...
        res.json({
            activeInstantMeeting: activeInstant[0] || null,
            notifications
        });
    } catch (err) {
        res.status(500).json({ error: err.message });
...
app.get("/api/meetings/:id", authenticate, async (req, res) => {
    try {
        ...
        const meeting = await Meeting.findOne({
            ...
            include: [
                { model: User, as: "host", attributes: ["id", "name", "email", "role", "designation"] },
                {
                    model: MeetingAttendance,
                    as: "attendances",
                    include: [{ model: User, as: "user", attributes: ["id", "name", "email", "role", "designation"] }]
                }
            ]
        });
        ...
        // Populate participants profile list
        let participantsIds = [];
        try {
            participantsIds = JSON.parse(meeting.participants || "[]");
        } catch (e) {}

        const participantsProfiles = await User.findAll({
            where: { id: participantsIds },
            attributes: ["id", "name", "email", "role", "designation"]
        });

        res.json({ meeting, participantsProfiles });
    } catch (err) {
        res.status(500).json({ error: err.message });
    }
});
...
app.post("/api/meetings/:id/join", authenticate, async (req, res) => {
    try {
        ...
        if (meeting.status === "completed" || meeting.status === "cancelled") {
            return res.status(400).json({ error: "This meeting is already closed." });
        }

        // If scheduled and host joins, start the meeting!
        if (meeting.status === "scheduled" && Number(meeting.hostId) === Number(userId)) {
            await meeting.update({ status: "live" });
            // Notify other participants that meeting has started
            try {
                const pts = JSON.parse(meeting.participants || "[]");
                for (const pId of pts) {
                    await Notification.create({
                        userId: pId,
                        type: "system_alert",
                        title: "Meeting Started",
                        message: `The meeting '${meeting.title}' has started. Click here to join now.`,
                        createdAt: new Date()
                    });
                }
            } catch (e) {}
        ...
        const now = new Date();
        if (!attendance) {
            // Determine if Late Joined
            let status = "joined";
            if (meeting.meetingType === "scheduled" && meeting.scheduledTime && meeting.scheduledDate) {
                try {
                    const scheduledDateTime = new Date(`${meeting.scheduledDate}T${meeting.scheduledTime}`);
                    const diffMins = (now - scheduledDateTime) / 60000;
                    if (diffMins > 5) {
                        status = "late_joined";
                    }
                } catch(e) {}
            ...
            // Update rejoin
            const rejoinsCount = (attendance.rejoins || 0) + 1;
...
// Reject meeting (For Boss/Manager/TL scheduler)
app.post("/api/meetings/:id/reject", authenticate, async (req, res) => {
    try {
        ...
        const meeting = await Meeting.findByPk(meetingId, {
            include: [{ model: User, as: "host", attributes: ["id", "name", "role"] }]
        });
        if (!meeting) {
            return res.status(404).json({ error: "Meeting not found" });
        }

        // Kicking/rejection rules enforcement
        // Boss cannot be forced to join anyone's meeting
        // Manager can reject TL meetings
        // Recruiter CANNOT reject any meetings
        const hostRole = meeting.host.role;
        let rejectAllowed = false;

        if (role === "boss") {
            rejectAllowed = true; // Boss can reject any meeting
        } else if (role === "manager" && hostRole === "tl") {
            rejectAllowed = true; // Manager can reject TL meeting
        } else if (role === "tl" && hostRole === "tl") {
            rejectAllowed = true; // TL can reject peer TL meeting
        } else if (role === "manager" && hostRole === "manager") {
            rejectAllowed = true; // Manager can reject peer Manager meeting
        }

        if (!rejectAllowed) {
            return res.status(403).json({ error: "Attendance is mandatory. You cannot reject this meeting invitation." });
        }
        ...
        if (!attendance) {
            attendance = await MeetingAttendance.create({
                ...
                userId,
                rejected: true,
                rejectReason: reason || "Unavailable",
                status: "absent"
            });
        } else {
            await attendance.update({
        ...
        await Notification.create({
            ...
            type: "system_alert",
            title: "Meeting Invitation Rejected",
            message: `${name} rejected your meeting: '${meeting.title}'. Reason: ${reason || "None specified"}.`,
            createdAt: new Date()
        });
...
app.post("/api/meetings/:id/leave", authenticate, async (req, res) => {
    try {
        ...
        const now = new Date();
        const joinTime = new Date(attendance.joinTime || attendance.createdAt || now);
        const sessionSecs = Math.floor((now - joinTime) / 1000);
        const totalDuration = (attendance.duration || 0) + sessionSecs;
        ...
        if (meeting && meeting.status === "live") {
            // User left early if meeting is still live
...
// Telemetry heartbeat updates
app.post("/api/meetings/:id/telemetry", authenticate, async (req, res) => {
    try {
        const { userId } = req.user;
        const meetingId = req.params.id;
        const { micActive, cameraActive, screenshareActive, disconnected } = req.body;
        ...
        // Ticks calculations (assume heartbeat is sent every 10 seconds)
        // Accumulate active usage percentages
        const disconnects = disconnected ? (attendance.networkDisconnects || 0) + 1 : (attendance.networkDisconnects || 0);

        // Simple running telemetry calculation:
        // We will increment based on weights or average values directly.
        const currentCam = attendance.cameraUsage || 0;
        const currentMic = attendance.micUsage || 0;
        const currentScreen = attendance.screenshareUsage || 0;

        const newCam = cameraActive ? Math.min(100, currentCam + 5) : currentCam;
        const newMic = micActive ? Math.min(100, currentMic + 5) : currentMic;
        const newScreen = screenshareActive ? Math.min(100, currentScreen + 5) : currentScreen;

        const isVidDisabled = attendance.videoDisabledByHost;
        const isMicMuted = attendance.mutedByHost;

        await attendance.update({
            cameraUsage: newCam,
            micUsage: newMic,
            screenshareUsage: newScreen,
            networkDisconnects: disconnects,
            status: disconnected ? "disconnected" : attendance.status,
            cameraActive: isVidDisabled ? false : !!cameraActive,
            micActive: isMicMuted ? false : !!micActive,
            screenshareActive: !!screenshareActive,
            videoDisabledByHost: isVidDisabled,
            mutedByHost: isMicMuted,
            lastHeartbeat: new Date()
        });

        // Sync meeting-level presentation state
        if (screenshareActive) {
            await Meeting.update(
                { presenterId: userId, sharedModule: req.body.sharedModule || null },
                { where: { id: meetingId } }
            );
        } else {
            await Meeting.update(
                { presenterId: null, sharedModule: null },
                { where: { id: meetingId, presenterId: userId } }
            );
        }
...
app.post("/api/meetings/:id/chat", authenticate, async (req, res) => {
    try {
...
app.get("/api/meetings/:id/chat", authenticate, async (req, res) => {
    try {
        const chats = await MeetingChat.findAll({
...
app.post("/api/meetings/:id/handraise", authenticate, async (req, res) => {
    try {
        const { userId } = req.user;
        const meetingId = req.params.id;
        ...
        await attendance.update({ handRaised: !!handRaised });
        res.json({ success: true, handRaised: !!handRaised });
    } catch (err) {
        res.status(500).json({ error: err.message });
    }
});

// Mute single participant
app.post("/api/meetings/:id/mute-user", authenticate, async (req, res) => {
    try {
        ...
        const isHost = Number(meeting.hostId) === Number(userId);
        if (!isHost && role !== "boss" && role !== "manager" && role !== "tl") {
            return res.status(403).json({ error: "Unauthorized to mute users." });
        }
        ...
        await attendance.update({
            mutedByHost: !!mute,
            micActive: mute ? false : attendance.micActive
        });
...
app.post("/api/meetings/:id/mute-all", authenticate, async (req, res) => {
    try {
        const { userId, role } = req.user;
        const meetingId = req.params.id;
        const { mute } = req.body;
        ...
        const isHost = Number(meeting.hostId) === Number(userId);
        if (!isHost && role !== "boss" && role !== "manager" && role !== "tl") {
            return res.status(403).json({ error: "Unauthorized to mute all." });
        }

        await MeetingAttendance.update(
            { 
                mutedByHost: !!mute,
                micActive: mute ? false : undefined 
            },
            {
                where: { 
                    meetingId,
                    userId: { [Op.ne]: meeting.hostId }
                }
            }
        );
...
// Disable single participant camera
app.post("/api/meetings/:id/disable-user-video", authenticate, async (req, res) => {
    try {
        const { userId, role } = req.user;
        const meetingId = req.params.id;
        ...
        const attendance = await MeetingAttendance.findOne({ where: { meetingId, userId: targetUserId } });
        if (!attendance) return res.status(404).json({ error: "Participant not found" });

        await attendance.update({ 
            videoDisabledByHost: !!disable,
            cameraActive: disable ? false : attendance.cameraActive 
        });
        res.json({ success: true });
    } catch (err) {
        res.status(500).json({ error: err.message });
...
// Disable all participant cameras (except host)
app.post("/api/meetings/:id/disable-all-video", authenticate, async (req, res) => {
    try {
        const { userId, role } = req.user;
        const meetingId = req.params.id;
        ...
        await MeetingAttendance.update(
            { cameraActive: false, videoDisabledByHost: true },
            { where: { meetingId, userId: { [Op.ne]: meeting.hostId } } }
        );
...
// Get online company users not already in this meeting
app.get("/api/meetings/:id/online-users", authenticate, async (req, res) => {
    try {
        const { companyId } = req.user;
        const meetingId = req.params.id;

        // Consider users online if lastSeen within last 90 seconds
        ...
        const onlineUsers = await User.findAll({
            where: {
                companyId,
                status: "active",
                lastSeen: { [Op.gte]: ninetySecondsAgo }
            },
            attributes: ["id", "name", "role", "lastSeen"]
        });

        // Get current meeting participant userIds from attendances
        const attendances = await MeetingAttendance.findAll({
            where: { meetingId, leaveTime: null, rejected: false },
            attributes: ["userId"]
        });
        const inMeetingIds = new Set(attendances.map(a => Number(a.userId)));

        const available = onlineUsers.filter(u => !inMeetingIds.has(Number(u.id)));
        res.json(available);
    } catch (err) {
        res.status(500).json({ error: err.message });
    }
...
// Invite an online user to join the meeting (creates notification)
app.post("/api/meetings/:id/invite-user", authenticate, async (req, res) => {
    try {
        const { userId, name } = req.user;
        const meetingId = req.params.id;
        ...
        // Update participants list of the meeting so the user is treated as invited
        let participantsIds = [];
        try {
            participantsIds = JSON.parse(meeting.participants || "[]");
        } catch (e) {}

        if (!participantsIds.map(Number).includes(Number(targetUserId))) {
            participantsIds.push(Number(targetUserId));
            await meeting.update({ participants: JSON.stringify(participantsIds) });
        }

        // Find existing attendance and reset it to allow re-invite popup alerts
        const attendance = await MeetingAttendance.findOne({
            where: { meetingId, userId: targetUserId }
        });
        if (attendance) {
        ...
        await Notification.create({
            ...
            message: `${name} is inviting you to join the live meeting: "${meeting.title}". Click to join.`,
            data: JSON.stringify({ meetingId, meetingTitle: meeting.title }),
            createdAt: new Date()
        });
...
// Kick participant
app.post("/api/meetings/:id/kick", authenticate, async (req, res) => {
    try {
        const { userId, role } = req.user;
        const meetingId = req.params.id;
        const { targetUserId } = req.body;
        ...
        const isHost = Number(meeting.hostId) === Number(userId);
        if (!isHost && role !== "boss" && role !== "manager" && role !== "tl") {
            return res.status(403).json({ error: "Unauthorized to kick users." });
        }
...
// Reschedule meeting
app.put("/api/meetings/:id/reschedule", authenticate, async (req, res) => {
    try {
        const { userId, name, role } = req.user;
        const meetingId = req.params.id;
        ...
        if (Number(meeting.hostId) !== Number(userId) && role !== "boss") {
            return res.status(403).json({ error: "Only the host can reschedule this meeting." });
        }

        let history = [];
        try {
            history = JSON.parse(meeting.rescheduleHistory || "[]");
        } catch (e) {}

        history.push({
            oldDate: meeting.scheduledDate,
            oldTime: meeting.scheduledTime,
            newDate,
            newTime,
            reason,
            timestamp: new Date()
        ...
        try {
            ...
            for (const pId of pts) {
                await Notification.create({
                    ...
                    message: `The meeting '${meeting.title}' has been rescheduled to ${newDate} at ${newTime}. Reason: ${reason}`,
                    createdAt: new Date()
                });
            }
        } catch (e) {}
...
// Cancel meeting
app.put("/api/meetings/:id/cancel", authenticate, async (req, res) => {
    try {
        const { userId, name, role } = req.user;
        const meetingId = req.params.id;
        const { reason } = req.body;
        ...
        try {
            ...
            for (const pId of pts) {
                await Notification.create({
                    ...
                    message: `The meeting '${meeting.title}' has been cancelled by ${name}. Reason: ${reason}`,
                    createdAt: new Date()
                });
            }
        } catch (e) {}
...
app.put("/api/meetings/:id/end", authenticate, async (req, res) => {
    try {
        const { userId } = req.user;
        const meetingId = req.params.id;
        ...
        // Host checks
        if (Number(meeting.hostId) !== Number(userId) && req.user.role !== "boss") {
            return res.status(403).json({ error: "Only the host can end this meeting." });
        }

        const now = new Date();

        // 1. Close all active meeting attendances
        const activeAttendances = await MeetingAttendance.findAll({
            where: { meetingId, leaveTime: null, rejected: false }
        });

        for (const att of activeAttendances) {
            const joinTime = new Date(att.joinTime || att.createdAt || now);
            const durationSecs = Math.floor((now - joinTime) / 1000);
            await att.update({
                leaveTime: now,
                duration: (att.duration || 0) + durationSecs,
                status: "completed"
            });
        }

        // 2. Mark any participants who never joined as absent
        let participantsIds = [];
        try {
            participantsIds = JSON.parse(meeting.participants || "[]");
        } catch (e) {}
        ...
        // 3. Generate high-fidelity AI simulated summary
        const summaryObj = {
            discussionSummary: `In this meeting, the host ${meeting.host.name} and team aligned on the operational roadmap regarding "${meeting.title}". The agenda points "${meeting.agenda || 'None'}" were thoroughly reviewed and detailed targets were assigned.`,
            keyPoints: [
                "Reviewed recruitment pipeline conversions for active clients.",
                "Discussed candidate retention rates and desking parameters.",
                "Addressed network latency and desking telemetry compliance."
            ],
            decisionsTaken: [
                "All recruiters must keep their visual dashboards open during calling cycles.",
                "Shift timings and late logins will be audited closely starting next Monday."
            ],
            assignedTasks: [
                {
                    title: `Audit candidates for ${meeting.title}`,
                    assignee: "Demonstration Recruiter",
                    deadline: new Date(Date.now() + 2 * 24 * 60 * 60 * 1000).toISOString().split("T")[0]
                },
                {
                    title: `Update performance matrix for ${meeting.agenda || 'Module'}`,
                    assignee: "Team Leader Node",
...
[00:01] ${meeting.host.name}: Welcome team. Let's discuss our core agenda regarding: ${meeting.agenda || meeting.title}.
[04:12] Participant Node 1: Yes, we are currently sourcing active candidates. Some profiles are under review.
[12:30] Participant Node 2: I suggest we create immediate tasks for the team leaders to keep dashboards synced.
[18:45] ${meeting.host.name}: Agreed. Let's close this call and push the summary items. Thanks everyone.
        `.trim();
        ...
        await meeting.update({
            ...
            recordingUrl: `/uploads/recordings/simulated_meet_${meeting.id}.mp4`
        });
    ...
    try {
        ...
        if (user) {
            if (!user.lastReadMessageId || lastMessageId > user.lastReadMessageId) {
                await user.update({ lastReadMessageId: lastMessageId });
            }
        }
        res.json({ success: true });
    } catch (err) {
...
app.get("/api/chat/:id/status", authenticate, async (req, res) => {
    try {
        ...
        const members = await User.findAll({
            ...
            attributes: ["id", "name", "role", "email", "lastReadMessageId", "lastSeen"]
        });
        ...
        members.forEach(m => {
            ...
            if (hasSeen) {
                ...
                const isOnline = (m.lastSeen && new Date(m.lastSeen) >= fiveMinsAgo) || 
                                 (att && att.logoutTime === null && !att.isIdle && !att.onBreak);
...
app.patch("/api/jobs/:id", authenticate, async (req, res) => {
    try {
        ...
        if (Array.isArray(updateData.clientIds) && updateData.clientIds.length > 0) {
            updateData.clientId = updateData.clientIds[0];
        }
        ...
        if (!job) {
            ...
            if (fallbackJob) {
                await fallbackJob.update(updateData);
...
function isTimeWithinShiftWindow(timeValue, shift) {
    ...
    // Parse timeValue into Asia/Kolkata timezone
    const d = new Date(new Date(timeValue).toLocaleString('en-US', { timeZone: 'Asia/Kolkata' }));
...
app.post("/api/shifts", authenticate, async (req, res) => {
    ...
    try {
        ...
        if (!shiftData.startDate) {
            return res.status(400).json({ error: "Shift Start Date is mandatory" });
        }
...
app.get("/api/attendance/hub", authenticate, async (req, res) => {
    ...
    try {
        ...
        if (startDate && endDate) {
            ...
            let count = 0;
            while (current <= last && count < 90) { // cap at 90 days to prevent overload
        ...
        // Batch query all attendance records for the target users on target dates
        const allAtts = await Attendance.findAll({
            where: {
                userId: { [Op.in]: employees.map(e => e.id) },
                date: { [Op.in]: targetDates }
            },
            include: [
                { model: Shift, as: "shift" },
                { model: AttendanceLog, as: "logs" },
        ...
        for (const emp of employees) {
            for (const d of targetDates) {
                const att = attMap[`${emp.id}_${d}`] || null;
                attendanceList.push({
                    id: `${emp.id}_${d}`,
                    date: d,
                    user: {
        ...
        // Sort by date descending (latest first) then by employee name
...
app.get("/api/candidates", authenticate, async (req, res) => {
    try {
        ...
        const candidates = await Candidate.findAll({
            where: { companyId: effectiveCompanyId },
            include: [
                { model: Note, as: "InteractionNotes" },
                { model: User, as: "recruiter", attributes: ["id", "name"] },
    ...
    try {
        ...
        if (req.user.role === "tl") {
            ...
            await User.update(
                ...
                { where: { role: "tl", reportingTo: null, companyId: req.user.companyId || null } }
            );
            await User.update(
                { reportingTo: req.user.userId },
                { where: { role: "recruiter", reportingTo: null, companyId: req.user.companyId || null } }
            );

            const myTls = await User.findAll({
                where: { role: "tl", reportingTo: req.user.userId, companyId: req.user.companyId || null },
                attributes: ["id"]
            });
...
app.get("/api/boss/team-monitoring", authenticate, async (req, res) => {
    let allowed = req.user.role === "boss";
    if (!allowed) {
        try {
            const activeAtt = await MeetingAttendance.findOne({
                where: { userId: req.user.id, status: 'joined', leaveTime: null }
            });
            if (activeAtt) allowed = true;
        } catch (err) {
            console.error("Meeting check failed in team-monitoring:", err);
        }
    }
    if (!allowed) {
    ...
    try {
        ...
        for (const u of users) {
            ...
            if (att) {
                ...
                // calculate working minutes and seconds
                if (att.logs) {
                    let activeMins = 0;
                    let activeSecs = 0;
                    for (const log of att.logs) {
                        ...
                        const logout = log.logoutTime ? new Date(log.logoutTime) : new Date();
                        activeMins += Math.max(0, Math.round((logout - login) / 60000));
                        activeSecs += Math.max(0, Math.floor((logout - login) / 1000));
                    }
                    ...
                    if (att.breaks) {
                        for (const b of att.breaks) {
                            ...
                            const endB = b.endTime ? new Date(b.endTime) : new Date();
                            breakSecs += Math.max(0, Math.floor((endB - startB) / 1000));
                        }
                    } else {
                        breakSecs = totalBreakTime * 60;
                    }
                    
                    liveWorkingMins = Math.max(0, activeMins - totalBreakTime);
                    liveWorkingSecs = Math.max(0, activeSecs - breakSecs);
                }
            }
            ...
            userList.push({
                ...
                workingHoursSecs: liveWorkingSecs,
                breakCount,
        ...
        const dbPlatforms = await SourcingPlatform.findAll({
            where: { companyId: req.user.companyId || null }
        });
        const platformCasingMap = new Map();
        dbPlatforms.forEach(p => {
            const trimmed = p.name.trim();
            if (trimmed) platformCasingMap.set(trimmed.toLowerCase(), trimmed);
        });
        allCandidates.forEach(c => {
            const trimmed = (c.sourcingBy || "Direct").trim();
            if (trimmed && !platformCasingMap.has(trimmed.toLowerCase())) {
                platformCasingMap.set(trimmed.toLowerCase(), trimmed);
            }
        });

        const sourcingBreakdown = {};
        platformCasingMap.forEach((originalName) => {
            sourcingBreakdown[originalName] = { registered: 0, connected: 0, interested: 0, selected: 0, joined: 0 };
        });

        allCandidates.forEach(c => {
            const ch = (c.sourcingBy || "Direct").trim();
            const matchedOriginal = platformCasingMap.get(ch.toLowerCase());
            if (matchedOriginal && sourcingBreakdown[matchedOriginal]) {
                sourcingBreakdown[matchedOriginal].registered++;
                if (c.remarks === "Connected") sourcingBreakdown[matchedOriginal].connected++;
                if (c.remarks === "Interested") sourcingBreakdown[matchedOriginal].interested++;
                if (c.status === "Hired" || c.remarks === "Selected") sourcingBreakdown[matchedOriginal].selected++;
                if (c.remarks && c.remarks.toLowerCase().includes("join")) sourcingBreakdown[matchedOriginal].joined++;
            }
        });
...
app.get("/api/boss/reports-hub", authenticate, async (req, res) => {
    ...
    if (!allowed) {
        try {
            ...
            console.error("Meeting check failed in reports-hub:", err);
        }
    }
    if (!allowed) {
    ...
    try {
        ...
        const categoryMap = {};
        // Initialize default ones to show in UI
        leadCats.forEach(cat => {
            categoryMap[cat] = { category: cat, count: 0, conversion: 0 };
        });

        filteredCandidates.forEach(c => {
            if (c.dataType === "lead") {
                let cat = c.sector || c.qualification || c.remarks || "Other";
                cat = cat.trim();
                if (cat) {
                    // Match case-insensitively with default list
                    const matchedDefault = leadCats.find(lc => lc.toLowerCase() === cat.toLowerCase());
                    const key = matchedDefault || (cat.charAt(0).toUpperCase() + cat.slice(1));
                    if (!categoryMap[key]) {
                        categoryMap[key] = { category: key, count: 0, conversion: 0 };
                    }
                    categoryMap[key].count++;
                    if (c.status === "Hired" || c.remarks === "Selected" || (c.remarks && c.remarks.toLowerCase().includes("join"))) {
                        categoryMap[key].conversion++;
                    }
                }
            }
        });
...
app.put("/api/team/:id/reassign", authenticate, async (req, res) => {
    ...
    try {
        ...
        if (supervisorId) {
            ...
            if (!supervisor) return res.status(400).json({ error: "Supervisor not found or belongs to a different company" });
        }
...
app.post("/api/upload", authenticate, async (req, res) => {
    try {
        ...
        if (!fileName || !base64Data) {
            try { fs.writeFileSync(path.resolve(__dirname, "../upload_error.txt"), "Error: Missing file details. Body keys: " + (req.body ? Object.keys(req.body).join(", ") : "null")); } catch (e) {}
        ...
        try { fs.writeFileSync(path.resolve(__dirname, "../upload_error.txt"), "Error inside /api/upload: " + err.message + "\nStack: " + err.stack); } catch (e) {}
...
// POST /api/meetings/:meetingId/signal — send a WebRTC signal to a user
app.post("/api/meetings/:meetingId/signal", async (req, res) => {
    try {
        const { meetingId } = req.params;
        const { from: fromUserId, to: toUserId, data } = req.body;
        if (!fromUserId || !toUserId || !data) return res.status(400).json({ error: "Missing fields" });
        await sequelize.query(
            `INSERT INTO webrtc_signals (meetingId, fromUserId, toUserId, data) VALUES (?, ?, ?, ?)`,
            { replacements: [String(meetingId), Number(fromUserId), Number(toUserId), JSON.stringify(data)] }
        );
        res.json({ ok: true });
    } catch (err) {
        res.status(500).json({ error: err.message });
...
// GET /api/meetings/:meetingId/signals — fetch & clear pending signals for current user
app.get("/api/meetings/:meetingId/signals", async (req, res) => {
    try {
        ...
        const [rows] = await sequelize.query(
            `SELECT id, fromUserId, data FROM webrtc_signals WHERE meetingId = ? AND toUserId = ? ORDER BY id ASC`,
            { replacements: [String(meetingId), Number(userId)] }
        );
        if (rows.length > 0) {
            const ids = rows.map(r => r.id);
            await sequelize.query(`DELETE FROM webrtc_signals WHERE id IN (${ids.join(",")})`);
        }
        const signals = rows.map(r => ({ from: r.fromUserId, data: JSON.parse(r.data) }));
        res.json(signals);
    } catch (err) {
        res.status(500).json({ error: err.message });
    }
...
// Fallback SPA routing - serve index.html for all non-API routes
app.get("*", (req, res) => {
