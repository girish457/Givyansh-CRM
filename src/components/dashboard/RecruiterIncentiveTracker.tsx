...
export default function RecruiterIncentiveTracker() {
  ...
  const checkForPopups = (user: any, activeActions: IncentiveAction[]) => {
    try {
      const acknowledged = JSON.parse(localStorage.getItem("fast_rms_acknowledged_popups_v1") || "[]");
  ...
  const dismissCelebration = () => {
    if (celebrationPopup) {
      ...
      localStorage.setItem("fast_rms_acknowledged_popups_v1", JSON.stringify(acknowledged));
