...
import { 
  ...
  LucideRefreshCw, LucideBarChart3, LucideListTodo
} from "lucide-react";
...
export default function ManagerReportsCenter() {
  ...
  const incentiveInsights = useMemo(() => {
    const actionsList = JSON.parse(localStorage.getItem("fast_rms_incentive_actions_v1") || "[]") as any[];
  ...
  if (loading) {
    return (
      <div style={{ display: "flex", height: "calc(100vh - 120px)", alignItems: "center", justifyContent: "center", color: "#6366f1" }}>
        <LucideRefreshCw className="animate-spin" size={42} />
      </div>
    );
  }
