...
export default function BossIncentiveHub() {
  ...
  const initIncentiveState = (bossUser: any) => {
    ...
    if (storedRules) {
      ...
      localStorage.setItem("fast_rms_incentive_rules_v1", JSON.stringify(activeRules));
    }
    ...
    if (storedActions) {
      ...
      activeActions = [];
      localStorage.setItem("fast_rms_incentive_actions_v1", JSON.stringify(activeActions));
    }
    setActions(activeActions);

    // Audits
    const storedAudit = localStorage.getItem("fast_rms_incentive_audit_v1");
    if (storedAudit) {
      ...
      setAuditLogs(logs);
      localStorage.setItem("fast_rms_incentive_audit_v1", JSON.stringify(logs));
    }
  };
  ...
  const handleApproveSuggested = (suggested: Omit<IncentiveAction, "id" | "status" | "date" | "auditTrail">) => {
    ...
    setActions(updated);
    localStorage.setItem("fast_rms_incentive_actions_v1", JSON.stringify(updated));
  ...
  const handleCreateRule = (e: React.FormEvent) => {
    ...
    setRules(updated);
    localStorage.setItem("fast_rms_incentive_rules_v1", JSON.stringify(updated));
  ...
  const addAuditLog = (event: string) => {
    ...
    localStorage.setItem("fast_rms_incentive_audit_v1", JSON.stringify(updated));
  };
