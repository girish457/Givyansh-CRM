...
export default function ManagerDashboardHome({ userProfile }: ManagerDashboardHomeProps) {
  ...
  const totals = useMemo(() => {
    ...
    const approvedIncentives = (JSON.parse(localStorage.getItem("fast_rms_incentive_actions_v1") || "[]") as any[])
      .filter((a: any) => a.status === "Approved")
      .reduce((sum: number, a: any) => sum + (a.actualAmount || 0), 0);

    const pendingApprovals = (JSON.parse(localStorage.getItem("fast_rms_incentive_actions_v1") || "[]") as any[])
