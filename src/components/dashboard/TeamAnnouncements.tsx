...
export default function TeamAnnouncements() {
  ...
  const [scheduleOption, setScheduleOption] = useState("specific"); // specific, tomorrow, weekly, monthly
  ...
  const fetchTeam = async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        if (data && Array.isArray(data.teamList)) {
  ...
  const toggleSelectRecruiter = (id: number) => {
  ...
  const selectAllTeam = () => {
    setSelectedRecruiters(teamMembers.map(m => m.id));
  };

  const selectOnlineOnly = () => {
    const onlineIds = teamMembers.filter(m => m.online).map(m => m.id);
    setSelectedRecruiters(onlineIds);
  };

  const selectShiftWise = (shiftName: string) => {
    const shiftIds = teamMembers
      .filter(m => m.shift?.name?.toLowerCase().includes(shiftName.toLowerCase()))
      .map(m => m.id);
    setSelectedRecruiters(shiftIds);
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (selectedRecruiters.length === 0) {
      showNotification("error", "Please select at least one recruiter recipient");
      return;
    }
    ...
    const payload = {
      ...
      targetAssignees: selectedRecruiters,
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: "14px", overflowY: "auto", paddingRight: "4px" }}>
        ...
        <div style={{ background: "#ffffff", border: "1px solid #e2e8f0", boxShadow: "0 4px 12px rgba(0,0,0,0.03)", borderRadius: "12px", padding: "14px 16px" }}>
          ...
          <form onSubmit={handleSubmit} style={{ display: "flex", flexDirection: "column", gap: "10px" }}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "3fr 1.2fr", gap: "10px" }}>
              <div>
                ...
                <input
                  ...
                  placeholder="e.g. Daily Target Update..."
            ...
            {/* POLL OPTIONS CONFIG (Only if poll or mixed) */}
            ...
            <div style={{ padding: "8px 10px", background: "#f8fafc", borderRadius: "6px", border: "1px solid #e2e8f0" }}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "6px" }}>
                <label style={{ fontSize: "10px", color: "#475569", display: "flex", alignItems: "center", gap: "4px", fontWeight: "700" }}>
                  <LucideUsers size={11} />
                  Recipients ({selectedRecruiters.length} Selected)
                </label>
                <div style={{ display: "flex", gap: "4px" }}>
                  ...
                  <button type="button" onClick={() => selectShiftWise("day")} style={{ background: "rgba(245,158,11,0.08)", border: "none", color: "#d97706", fontSize: "9.5px", padding: "1px 5px", borderRadius: "3px", cursor: "pointer", fontWeight: "700" }}>Day</button>
                  <button type="button" onClick={() => selectShiftWise("night")} style={{ background: "rgba(139,92,246,0.08)", border: "none", color: "#7c3aed", fontSize: "9.5px", padding: "1px 5px", borderRadius: "3px", cursor: "pointer", fontWeight: "700" }}>Night</button>
              ...
              {loadingTeam ? (
                ...
                <div style={{ fontSize: "10px", color: "#64748b", textAlign: "center", padding: "6px" }}>No recruiters under hierarchy.</div>
              ) : (
                <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: "4px", maxHeight: "90px", overflowY: "auto", padding: "1px" }}>
                  {teamMembers.map(m => (
                    <div
                      key={m.id}
                      onClick={() => toggleSelectRecruiter(m.id)}
                      style={{
                        background: selectedRecruiters.includes(m.id) ? "rgba(37, 99, 235, 0.06)" : "#ffffff",
                        border: `1px solid ${selectedRecruiters.includes(m.id) ? "#3b82f6" : "#cbd5e1"}`,
                        borderRadius: "4px",
                        padding: "4px 6px",
                        cursor: "pointer",
                        display: "flex",
                    ...
                    >
                      <div style={{ display: "flex", alignItems: "center", gap: "4px" }}>
                        <span style={{
                          ...
                          height: "5px",
                          borderRadius: "50%",
                          background: m.online ? "#10b981" : "#64748b"
                        }} />
                        <span style={{ fontSize: "10.5px", color: "#1e293b", fontWeight: selectedRecruiters.includes(m.id) ? "700" : "500" }}>
                          {m.name.split(" ")[0]}
                        </span>
                      </div>
                      {selectedRecruiters.includes(m.id) && <LucideCheck size={9} style={{ color: "#2563eb", strokeWidth: 3 }} />}
                    </div>
                  ))}
                </div>
              )}
          ...
          <div style={{ background: "#ffffff", border: "1px solid #cbd5e1", boxShadow: "0 4px 16px rgba(0,0,0,0.06)", borderRadius: "12px", padding: "14px 16px", display: "flex", flexDirection: "column", gap: "12px" }}>
            ...
            {loadingAnalytics ? (
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: "12px" }}>
                ...
                {/* Poll visual metrics (Only if poll or mixed) */}
                ...
                {/* Team Seen & Response Status Checklist */}
                <div>
                    ...
                    {analyticsData.responses.map((res: any) => {
                      ...
                      return (
                        <div key={res.id} style={{ background: "#ffffff", border: "1px solid #e2e8f0", borderRadius: "4px", padding: "6px", display: "flex", flexDirection: "column", gap: "3px" }}>
                          ...
                          {/* Poll selection or manual answers */}
