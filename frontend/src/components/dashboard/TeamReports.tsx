...
import { 
  ...
  LucideGlobe, LucideX, LucideLoader2
} from "lucide-react";
...
export default function TeamReports() {
  ...
  const [selectedSheetRecruiter, setSelectedSheetRecruiter] = useState<any>(null);
  ...
  useEffect(() => {
    fetchCandidatesAndVendors();
  }, []);

  const fetchCandidatesAndVendors = async () => {
    try {
      const [candRes, vendRes, meRes] = await Promise.all([
        fetch("/api/candidates"),
        fetch("/api/vendors"),
        fetch("/api/me")
      ]);
      if (candRes.ok) setCandidates(await candRes.json());
      if (vendRes.ok) setDbVendors(await vendRes.json());
      if (meRes.ok) setCurrentUser(await meRes.json());
    } catch (err) {
      console.error("Failed to fetch initial reports data:", err);
    }
  };

  const getLineupCandidatesForRecruiter = (rec: any) => {
    if (!rec) return [];
    const today = new Date();
    
    // 1. Filter by recruiter ID or name
    const recId = String(rec.id);
    const recName = (rec.name || "").toLowerCase();

    const recruiterCandidates = candidates.filter((c: any) => {
      const matchDbId = String(c.addedBy) === recId || String(c.assignedTo) === recId;
      const matchName = c.recruiterName && c.recruiterName.toLowerCase() === recName;
      return matchDbId || matchName;
    });

    // 2. Filter by Date Period (matching dateMode state of TeamReports.tsx)
    return recruiterCandidates.filter((c: any) => {
  ...
  const getRecruiterName = (c: any) => {
    if (c.recruiterName) return c.recruiterName;
    if (selectedSheetRecruiter) return selectedSheetRecruiter.name;
    return "N/A";
  };

  const getSeniorName = (c: any) => {
  ...
  return (
    <div className="reports-tab-container p-medium print-style-reports" style={{ height: "100%", overflowY: "auto", background: "#f8fafc", padding: "0.5rem", fontFamily: "'Outfit', 'Inter', sans-serif" }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr", gap: "8px" }}>
        ...
        {loading ? (
          <div style={{ display: "flex", justifyContent: "center", alignItems: "center", padding: "2rem", flexDirection: "column", gap: "12px" }}>
            <LucideLoader2 className="animate-spin" size={40} color="#2563eb" />
            <span style={{ fontWeight: 800, color: "#475569", fontSize: "0.78rem" }}>Compiling Real-Time Recruiter Roster...</span>
          </div>
                ...
                >
                  ...
                  <div style={{ display: "flex", gap: "14px", alignItems: "center" }}>
                    ...
                    <button
                      onClick={(e) => {
                        e.stopPropagation();
                        setSelectedSheetRecruiter(r.recruiter);
                        setShowLineupSheet(true);
                      }}
                    ...
                    >
                      <LucideFileText size={12} /> View Lineup Sheet
                    </button>
      ...
      {/* View Lineup Sheet Modal */}
      <AnimatePresence>
        {showLineupSheet && selectedSheetRecruiter && (
          <div style={{ position: "fixed", inset: 0, zIndex: 99999, background: "rgba(15, 23, 42, 0.5)", backdropFilter: "blur(4px)", display: "flex", justifyContent: "center", alignItems: "center" }}>
            <motion.div
              initial={{ scale: 0.95, opacity: 0 }}
              animate={{ scale: 1, opacity: 1 }}
              exit={{ scale: 0.95, opacity: 0 }}
            ...
            >
              ...
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "1.5rem", borderBottom: "1px solid #f1f5f9", paddingBottom: "12px" }}>
                <div>
                  <h3 style={{ fontSize: "1.25rem", fontWeight: 800, color: "#0f172a", margin: 0, display: "flex", alignItems: "center", gap: "8px" }}>
                    <LucideFileText size={20} color="#2563eb" /> Lineup Sheet: {selectedSheetRecruiter.name}
                  </h3>
                  <p style={{ color: "#64748b", fontSize: "0.82rem", margin: "4px 0 0 0" }}>
                    Showing {getLineupCandidatesForRecruiter(selectedSheetRecruiter).length} candidates sourced by this recruiter.
                  </p>
                </div>
                <button
                  onClick={() => { setShowLineupSheet(false); setSelectedSheetRecruiter(null); }}
                  style={{
                    background: "#f1f5f9",
                    border: "none",
                    borderRadius: "50%",
                ...
                >
                  <LucideX size={18} color="#64748b" />
                </button>
              </div>

              {/* Table Container */}
              <div className="custom-scrollbar" style={{ flex: 1, overflowY: "auto", overflowX: "auto", border: "1px solid #e2e8f0", borderRadius: "12px", background: "#f8fafc" }}>
                {getLineupCandidatesForRecruiter(selectedSheetRecruiter).length === 0 ? (
                  <div style={{ padding: "4rem 2rem", textAlign: "center", color: "#94a3b8" }}>
                    <LucideFileText size={48} style={{ opacity: 0.3, marginBottom: "1rem" }} />
                    <p style={{ fontWeight: 700, fontSize: "0.95rem", margin: 0 }}>No candidates found</p>
                    <p style={{ fontSize: "0.8rem", margin: "4px 0 0 0" }}>No sourced candidates logged for this recruiter in the selected date period.</p>
                  </div>
                ) : (
                  <table style={{ width: "100%", borderCollapse: "collapse", textAlign: "left", fontSize: "0.8rem" }}>
                    <thead>
                      <tr style={{ background: "#f1f5f9", borderBottom: "2px solid #e2e8f0", position: "sticky", top: 0, zIndex: 1 }}>
                        <th style={{ padding: "12px 16px", fontWeight: 800, color: "#475569", whiteSpace: "nowrap" }}>DATE</th>
                        <th style={{ padding: "12px 16px", fontWeight: 800, color: "#475569", whiteSpace: "nowrap" }}>NAME</th>
                        <th style={{ padding: "12px 16px", fontWeight: 800, color: "#475569", whiteSpace: "nowrap" }}>NUMBER</th>
                        <th style={{ padding: "12px 16px", fontWeight: 800, color: "#475569", whiteSpace: "nowrap" }}>LOCATION</th>
                        <th style={{ padding: "12px 16px", fontWeight: 800, color: "#475569", whiteSpace: "nowrap" }}>CLIENT</th>
                        <th style={{ padding: "12px 16px", fontWeight: 800, color: "#475569", whiteSpace: "nowrap" }}>RECRUITER</th>
                        <th style={{ padding: "12px 16px", fontWeight: 800, color: "#475569", whiteSpace: "nowrap" }}>REFERENCE</th>
                        <th style={{ padding: "12px 16px", fontWeight: 800, color: "#475569", whiteSpace: "nowrap" }}>STATUS</th>
                        <th style={{ padding: "12px 16px", fontWeight: 800, color: "#475569", whiteSpace: "nowrap" }}>REMARK</th>
                        <th style={{ padding: "12px 16px", fontWeight: 800, color: "#475569", whiteSpace: "nowrap" }}>DOJ</th>
                        <th style={{ padding: "12px 16px", fontWeight: 800, color: "#475569", whiteSpace: "nowrap" }}>REMARK BY</th>
                      </tr>
                    </thead>
                    <tbody>
                      {getLineupCandidatesForRecruiter(selectedSheetRecruiter).map((c, idx) => {
                        ...
                        const dojStr = hasJoined ? (c.updatedAt ? String(c.updatedAt).split("T")[0] : String(c.createdAt || "").split("T")[0]) : "N/A";
                        ...
                        return (
                          ...
                          >
                            <td style={{ padding: "12px 16px", color: "#334155", fontWeight: 600, whiteSpace: "nowrap" }}>{sourcingDateStr}</td>
                            <td style={{ padding: "12px 16px", color: "#0f172a", fontWeight: 700, whiteSpace: "nowrap" }}>{c.name || "N/A"}</td>
                            <td style={{ padding: "12px 16px", color: "#2563eb", fontWeight: 600, whiteSpace: "nowrap" }}>{c.phone || "N/A"}</td>
                            <td style={{ padding: "12px 16px", color: "#475569", whiteSpace: "nowrap" }}>{loc}</td>
                            <td style={{ padding: "12px 16px", color: "#475569", fontWeight: 600, whiteSpace: "nowrap" }}>{c.clientName || "N/A"}</td>
                            <td style={{ padding: "12px 16px", color: "#475569", whiteSpace: "nowrap" }}>{getRecruiterName(c)}</td>
                            <td style={{ padding: "12px 16px", color: "#475569", whiteSpace: "nowrap" }}>{getVendorName(c)}</td>
                            <td style={{ padding: "12px 16px", whiteSpace: "nowrap" }}>
                              <span style={{
                                ...
                                fontSize: "0.72rem",
                                fontWeight: 700,
                                background: hasJoined ? "#dcfce7" : "#e0f2fe",
                                color: hasJoined ? "#166534" : "#0369a1",
                                textTransform: "uppercase"
                              }}>
                                {remarkStatusStr}
                              </span>
                            </td>
                            <td style={{ padding: "12px 16px", color: "#475569", maxWidth: "200px", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }} title={c.remarkReason || c.remarks || "N/A"}>
                              {c.remarkReason || c.remarks || "N/A"}
                            </td>
                            <td style={{ padding: "12px 16px", color: "#16a34a", fontWeight: 700, whiteSpace: "nowrap" }}>{dojStr}</td>
                            <td style={{ padding: "12px 16px", color: "#475569", fontWeight: 600, whiteSpace: "nowrap" }}>{getSeniorName(c)}</td>
                          </tr>
                        );
                      })}
                    </tbody>
              ...
              {/* Footer */}
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginTop: "1.5rem", borderTop: "1px solid #f1f5f9", paddingTop: "12px", flexWrap: "wrap", gap: "10px" }}>
                <span style={{ fontSize: "0.8rem", color: "#64748b", fontWeight: 600 }}>
                  Active Audit Filter Summary: Date Filter Mode <strong style={{ color: "#0f172a" }}>{dateMode.toUpperCase()}</strong>
                </span>
                <button
                  onClick={() => { setShowLineupSheet(false); setSelectedSheetRecruiter(null); }}
                  style={{
