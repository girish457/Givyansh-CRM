...
export default function FeedbackTab({ currentUser }: FeedbackTabProps) {
  ...
  // Navigation
  const [historyTab, setHistoryTab] = useState<"Sent" | "Read" | "Replied" | "Closed">("Sent");
  ...
  const handleSelectFeedback = async (feedback: any) => {
    ...
    setNewReplyText("");
    setReplyAnonymous(feedback.isAnonymous);
    try {
  ...
  const handleSendReply = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch(`/api/feedback/${selectedFeedback.id}/reply`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ message: newReplyText, isAnonymous: replyAnonymous })
      });
  ...
  const filteredHistory = myHistory.filter(f => {
    if (historyTab === "Closed") return f.isClosed;
  ...
  if (loadingEligibility) {
    return (
      <div style={{ display: "flex", justifyContent: "center", alignItems: "center", height: "calc(100vh - 120px)", background: "#f8fafc", flexDirection: "column", gap: "15px" }}>
        ...
        <span style={{ fontSize: "1.1rem", fontWeight: 800, color: "#64748b" }}>Loading Daily Feedback...</span>
      </div>
    );
  }
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: "12px", background: "white", border: "1px solid #e2e8f0", borderRadius: "12px", padding: "16px", boxShadow: "0 4px 20px -3px rgba(0, 0, 0, 0.05)", height: "100%", overflowY: "auto" }}>
        ...
        {selectedFeedback ? (
          ...
          <div style={{ display: "flex", flexDirection: "column", height: "100%", gap: "12px" }}>
                  ...
                  return (
                    ...
                    >
                      <span style={{ fontSize: "0.64rem", fontWeight: 700, opacity: 0.85, marginBottom: "2px" }}>
                        {isMe ? (reply.isAnonymous ? "Anonymous (You)" : (reply.sender?.name || "You")) : `${reply.sender?.name} (${reply.senderRole.toUpperCase()})`}
                      </span>
            ...
            {/* REPLY COMPOSER FORM */}
            <form onSubmit={handleSendReply} style={{ display: "flex", gap: "6px", alignItems: "center" }}>
              <input
                type="text"
                disabled={selectedFeedback.isClosed}
                value={newReplyText}
                onChange={e => setNewReplyText(e.target.value)}
                placeholder={selectedFeedback.isClosed ? "Replies are disabled for closed queries." : "Type reply message..."}
                style={{
                  flex: 1,
                  padding: "8px 12px",
                  borderRadius: "8px",
                  ...
                  fontSize: "0.78rem",
                  outline: "none",
                  background: selectedFeedback.isClosed ? "#f1f5f9" : "white"
                }}
              />
              ...
              {!selectedFeedback.isClosed && (
                <button
                  ...
                  style={{
                    padding: "8px 12px",
                    borderRadius: "8px",
                    border: replyAnonymous ? "1px solid #7c3aed" : "1px solid #cbd5e1",
                    background: replyAnonymous ? "#f5f3ff" : "#f8fafc",
                    color: replyAnonymous ? "#7c3aed" : "#64748b",
                    cursor: "pointer",
                    display: "flex",
                    alignItems: "center",
                    gap: "6px",
                  ...
                  title={replyAnonymous ? "Replying anonymously" : "Replying with real name"}
                >
                  {replyAnonymous ? (
                    <>
                      <LucideUserX size={14} />
                    ...
                    <>
                      ...
                      <span>Identity: {currentUser?.name || "Real Name"}</span>
                    </>
                  )}
                </button>
              )}
              ...
              <button
                type="submit"
                disabled={sendingReply || selectedFeedback.isClosed}
                style={{
                  padding: "8px 16px",
                  borderRadius: "8px",
                  background: selectedFeedback.isClosed ? "#cbd5e1" : "linear-gradient(135deg, #3b82f6 0%, #2563eb 100%)",
                  ...
                  fontSize: "0.78rem",
                  fontWeight: 700,
                  boxShadow: selectedFeedback.isClosed ? "none" : "0 2px 6px rgba(37,99,235,0.2)"
                }}
              >
          ...
          <div style={{ display: "flex", flexDirection: "column", height: "100%", gap: "12px" }}>
            ...
            <div style={{ display: "flex", background: "#f1f5f9", padding: "3px", borderRadius: "8px", gap: "4px" }}>
              {(["Sent", "Read", "Replied", "Closed"] as const).map(tab => {
                const active = historyTab === tab;
                const count = tab === "Closed"
                  ? myHistory.filter(f => f.isClosed).length
                  : myHistory.filter(f => !f.isClosed && f.status === tab).length;
                return (
                  <button
                    key={tab}
