...
export default function TLReportingAI({ currentUser, candidates = [], onRefresh }: TLReportingAIProps) {
  ...
  const [includeMyReport, setIncludeMyReport] = useState(false);
  ...
  const getSeniorName = (c: any) => {
    const recId = c.addedBy || c.assignedTo;
    if (recId) {
      const rec = recruiters.find((r: any) => String(r.id) === String(recId));
      if (rec) {
        if (rec.manager_tl && rec.manager_tl.name) {
          return rec.manager_tl.name;
        }
      }
    }
  ...
  const filterCandidatesByRecruiterAndPeriod = () => {
    ...
    const teamCandidates = candidates.filter((c: any) => {
      if (includeMyReport) {
        const tlId = String(currentUser?.id || currentUser?.userId || "");
        const tlName = (currentUser?.name || "").toLowerCase();
        const isTlCandidate = String(c.addedBy) === tlId || String(c.assignedTo) === tlId || (c.recruiterName && c.recruiterName.toLowerCase() === tlName);
        if (isTlCandidate) return true;
      }
    ...
    <div className="module-container" style={{ position: "relative", height: "100%", overflowY: "auto", background: "#f8fafc", padding: "1.25rem", fontFamily: "'Outfit', 'Inter', sans-serif" }}>
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "1.25rem", flexWrap: "wrap", gap: "10px" }}>
        ...
        <div style={{ display: "flex", gap: "10px" }}>
          ...
          >
            <LucideFileBarChart size={14} /> View Lineup Sheet
          </button>
      ...
      <div style={{ background: "white", padding: "1rem", borderRadius: "16px", border: "1px solid #cbd5e1", display: "flex", flexDirection: "column", gap: "10px", marginBottom: "1.25rem" }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", borderBottom: "1px solid #f1f5f9", paddingBottom: "6px" }}>
          ...
          <div style={{ display: "flex", gap: "8px" }}>
            ...
            <button
              onClick={() => { setSelectedRecruiterIds([]); setIncludeMyReport(false); }}
        ...
        <div style={{ display: "flex", flexWrap: "wrap", gap: "10px", alignItems: "center" }}>
          <label
            style={{
              ...
              alignItems: "center",
              gap: "6px",
              background: includeMyReport ? "#eff6ff" : "#ffffff",
              border: `1.5px solid ${includeMyReport ? "#2563eb" : "#cbd5e1"}`,
              padding: "4px 10px",
              borderRadius: "8px",
              cursor: "pointer",
              fontSize: "0.78rem",
              fontWeight: includeMyReport ? 800 : 500,
              userSelect: "none",
              transition: "all 0.15s",
              color: includeMyReport ? "#2563eb" : "#475569"
            }}
          >
            <input
              type="checkbox"
              checked={includeMyReport}
              onChange={e => setIncludeMyReport(e.target.checked)}
            ...
            >
              ...
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "1.5rem", borderBottom: "1px solid #f1f5f9", paddingBottom: "12px" }}>
                <div>
                  <h3 style={{ fontSize: "1.25rem", fontWeight: 800, color: "#0f172a", margin: 0, display: "flex", alignItems: "center", gap: "8px" }}>
                    <LucideFileBarChart size={20} color="#2563eb" /> Lineup Sheet
                  </h3>
                  <p style={{ color: "#64748b", fontSize: "0.82rem", margin: "4px 0 0 0" }}>
                    Showing {filteredCandidates.length} filtered candidate records.
                  </p>
                </div>
                <button
                  onClick={() => setShowLineupSheet(false)}
                  style={{
                    background: "#f1f5f9",
                    border: "none",
                    borderRadius: "50%",
              ...
              <div className="custom-scrollbar" style={{ flex: 1, overflowY: "auto", overflowX: "auto", border: "1px solid #e2e8f0", borderRadius: "12px", background: "#f8fafc" }}>
                {filteredCandidates.length === 0 ? (
                  <div style={{ padding: "4rem 2rem", textAlign: "center", color: "#94a3b8" }}>
                    <LucideFileBarChart size={48} style={{ opacity: 0.3, marginBottom: "1rem" }} />
                    <p style={{ fontWeight: 700, fontSize: "0.95rem", margin: 0 }}>No candidates found</p>
                    <p style={{ fontSize: "0.8rem", margin: "4px 0 0 0" }}>Try adjusting the recruiters or date filters.</p>
                  </div>
                ) : (
              ...
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginTop: "1.5rem", borderTop: "1px solid #f1f5f9", paddingTop: "12px", flexWrap: "wrap", gap: "10px" }}>
                <span style={{ fontSize: "0.8rem", color: "#64748b", fontWeight: 600 }}>
                  Active Audit Filter Summary: <strong style={{ color: "#0f172a" }}>{dateFilter.toUpperCase()}</strong> with <strong style={{ color: "#0f172a" }}>{selectedRecruiterIds.length} Recruiters Selected</strong>
                </span>
                <button
                  onClick={() => setShowLineupSheet(false)}
                  style={{
