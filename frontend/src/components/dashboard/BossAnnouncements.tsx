...
export default function BossAnnouncements() {
  // Team (Managers, TLs & Recruiters) state
  ...
  // Recipient selectors (Manager, TL & Recruiter IDs)
  ...
  const fetchTeam = async () => {
    setLoadingTeam(true);
    try {
      // Pulls direct and indirect reporting structure from Boss scope
      const res = await fetch("/api/team");
      if (res.ok) {
        const data = await res.json();
        if (Array.isArray(data)) {
          // Keep Managers, TLs and Recruiters (filter out the boss node)
          const filtered = data.filter((m: any) => m.role === "manager" || m.role === "tl" || m.role === "recruiter");
  ...
  const selectManagersOnly = () => {
    const mgrIds = teamMembers.filter(m => m.role === "manager").map(m => m.id);
    setSelectedRecipients(mgrIds);
  };
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: "14px", overflowY: "auto", paddingRight: "4px" }}>
        ...
        <div style={{ background: "#ffffff", border: "1px solid #e2e8f0", boxShadow: "0 4px 12px rgba(0,0,0,0.03)", borderRadius: "12px", padding: "14px 16px" }}>
          <h2 style={{ fontSize: "14px", fontWeight: "700", color: "#0f172a", margin: "0 0 12px 0", display: "flex", alignItems: "center", gap: "8px" }}>
            ...
            Launch Announcement Broadcast (Boss Console)
          </h2>
            ...
            <div style={{ padding: "8px 10px", background: "#f8fafc", borderRadius: "6px", border: "1px solid #e2e8f0" }}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "6px" }}>
                ...
                <div style={{ display: "flex", gap: "4px" }}>
                  ...
                  <button type="button" onClick={selectManagersOnly} style={{ background: "rgba(16,185,129,0.08)", border: "none", color: "#10b981", fontSize: "9.5px", padding: "1px 5px", borderRadius: "3px", cursor: "pointer", fontWeight: "700" }}>Mgrs</button>
              ...
              {loadingTeam ? (
                <div style={{ display: "flex", justifyContent: "center", alignItems: "center", padding: "10px", gap: "6px" }}>
                  <LucideLoader2 className="animate-spin" size={14} color="#2563eb" />
                  <span style={{ fontSize: "10px", color: "#64748b" }}>Loading team roster...</span>
                </div>
                  ...
                  {teamMembers.map(m => (
                    ...
                    >
                      <div style={{ display: "flex", alignItems: "center", gap: "2px", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
                        <span style={{
                          fontSize: "8px",
                          background: m.role === "manager" ? "#f0fdf4" : m.role === "tl" ? "#f5f3ff" : "#eff6ff",
                          color: m.role === "manager" ? "#16a34a" : m.role === "tl" ? "#7c3aed" : "#2563eb",
                          padding: "1px 3px",
                          borderRadius: "3px",
                          fontWeight: "800",
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: "14px", overflowY: "auto", paddingRight: "4px" }}>
        ...
        <div style={{ background: "#ffffff", border: "1px solid #e2e8f0", boxShadow: "0 4px 12px rgba(0,0,0,0.03)", borderRadius: "12px", padding: "14px 16px", flex: 1, display: "flex", flexDirection: "column", minHeight: "300px", overflow: "hidden" }}>
          ...
          {loadingAnnouncements ? (
            <div style={{ display: "flex", flex: 1, alignItems: "center", justifyContent: "center", color: "#64748b", fontSize: "12px", gap: "8px", flexDirection: "column" }}>
              <LucideLoader2 className="animate-spin" size={24} color="#2563eb" />
        ...
        {selectedAnnouncement && (
          <div style={{ background: "#ffffff", border: "1px solid #cbd5e1", boxShadow: "0 4px 16px rgba(0,0,0,0.06)", borderRadius: "12px", padding: "14px 16px", display: "flex", flexDirection: "column", gap: "12px" }}>
            ...
            {loadingAnalytics ? (
              <div style={{ display: "flex", justifyContent: "center", alignItems: "center", padding: "15px", gap: "6px" }}>
                <LucideLoader2 className="animate-spin" size={14} color="#2563eb" />
                <span style={{ fontSize: "11px", color: "#64748b" }}>Compiling metrics telemetry...</span>
              </div>
            ) : analyticsData ? (
