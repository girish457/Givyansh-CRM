...
export default function BossIncentiveHub() {
  ...
  const initIncentiveState = (bossUser: any) => {
    const cid = bossUser?.companyId || "default";
    ...
    if (storedAudit) {
      ...
      const logs = [
        { timestamp: new Date().toISOString(), event: "Incentive Hub initialized on real database parameters", user: bossUser?.name || "Boss" }
      ];
      setAuditLogs(logs);
    ...
    if (storedBudget) {
      ...
      setTotalBudget(0);
      localStorage.setItem(bKey, "0");
    }
  };

  // Managers List
  const managers = useMemo(() => {
    return team.filter(t => t.role === "manager");
  }, [team]);

  // Team Hierarchy based on selected manager
  const myTls = useMemo(() => {
    if (selectedManagerId === "All") {
      return team.filter(t => t.role === "tl");
    }
    const mgrId = Number(selectedManagerId);
    return team.filter(t => t.role === "tl" && t.reportingTo === mgrId);
  }, [team, selectedManagerId]);
  ...
  }, [team, selectedManagerId, myTlIds]);

  const allMySubordinates = useMemo(() => [...myTls, ...myRecruiters], [myTls, myRecruiters]);

  // Filter actions based on subordinates of selected manager
  const visibleActions = useMemo(() => {
    if (selectedManagerId === "All") return actions;
    const subIds = new Set(allMySubordinates.map(s => s.id));
    subIds.add(Number(selectedManagerId)); // Include manager's own actions
    return actions.filter(act => subIds.has(act.employeeId));
  }, [actions, allMySubordinates, selectedManagerId]);
  ...
  // Suggested eligibility detector
  ...
  }, [visibleActions, suggestedIncentives, myRecruiters, myTls]);
  ...
  const leaderboardRankings = useMemo(() => {
    ...
    return {
      ...
      topTeams: myTls.map(t => {
        ...
        const teamEarnings = visibleActions
  ...
  }, [allMySubordinates, realTimePerformance, visibleActions, team, myTls]);
  ...
  const handleApproveSuggested = (suggested: Omit<IncentiveAction, "id" | "status" | "date" | "auditTrail">) => {
    const newAction: IncentiveAction = {
      ...
      approvedBy: currentUser?.name || "Boss",
      auditTrail: [{ timestamp: new Date().toISOString(), action: `Authorized milestone: ${suggested.achievement}`, user: currentUser?.name || "Boss" }]
    };
    ...
    const updated = actions.map(act => {
      if (act.id === actionId) {
        return {
          ...
          approvedBy: currentUser?.name || "Boss",
          auditTrail: [...act.auditTrail, { timestamp: new Date().toISOString(), action: "Approved manually", user: currentUser?.name || "Boss" }]
        };
      }
      return act;
    });
    ...
    const updated = actions.map(act => {
      if (act.id === actionId) {
        return {
          ...
          auditTrail: [...act.auditTrail, { timestamp: new Date().toISOString(), action: "Rejected manually", user: currentUser?.name || "Boss" }]
        };
      }
      return act;
    });
  ...
  const filteredHistory = useMemo(() => {
    return visibleActions.filter(act => {
  ...
  }, [visibleActions, historySearch, historyDateFilter, activeKpiFilter]);
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", borderBottom: "1px solid #e2e8f0", paddingBottom: "8px", marginBottom: "12px" }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: "10px" }}>
          <div style={{ display: "flex", alignItems: "center", gap: "6px" }}>
            <span style={{ fontSize: "0.7rem", fontWeight: 800, color: "#64748b" }}>Filter Manager:</span>
            <select
          ...
          return (
            <div
              ...
              style={{
                ...
                padding: "8px 10px",
                cursor: card.id === "Budget" ? "default" : "pointer",
                transition: "all 0.15s ease",
                boxShadow: "0 1px 2px rgba(0,0,0,0.02)"
              }}
            >
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                <span style={{ fontSize: "0.58rem", fontWeight: 700, color: "#64748b", textTransform: "uppercase" }}>{card.title}</span>
                {card.id === "Budget" && (
                  <button
                    onClick={(e) => {
                      e.stopPropagation();
              ...
              {card.id === "Budget" && isEditingBudget ? (
                <div style={{ display: "flex", gap: "4px", marginTop: "2px", alignItems: "center" }} onClick={(e) => e.stopPropagation()}>
                  <input
                    type="number"
                    value={tempBudget}
                  ...
                  <button
                    onClick={() => {
                      const newB = Number(tempBudget) || 0;
                      setTotalBudget(newB);
                      localStorage.setItem(`fast_rms_incentive_budget_v1_${companyId}`, String(newB));
                      setIsEditingBudget(false);
                      addAuditLog(`Updated corporate incentive budget limit to ₹${newB.toLocaleString()}`);
                    }}
                    style={{
                      background: "#10b981",
                      color: "white",
        ...
        {activeTab === "approvals" && (
          <div>
            <table style={{ width: "100%", borderCollapse: "collapse", fontSize: "0.68rem" }}>
              ...
              <tbody>
                {visibleActions.filter(x => x.status === "Pending").map(act => (
                ...
                {visibleActions.filter(x => x.status === "Pending").length === 0 && (
                  <tr>
