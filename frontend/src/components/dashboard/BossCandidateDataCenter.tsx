...
import { 
  ...
  LucideSparkles, LucideLayers, LucideGauge, LucidePencil, LucideChevronDown, LucideLoader2
} from "lucide-react";
...
export default function BossCandidateDataCenter() {
  ...
  // View States
  const [activeTab, setActiveTab] = useState<"all" | "manager" | "recruiter" | "tl" | "combined" | "analytics">("all");
  ...
  useEffect(() => {
    fetchProfile();
    fetchTeamAndData();
    const interval = setInterval(fetchTeamAndData, 8000);
  ...
  const filteredCandidates = useMemo(() => {
    return candidates.filter(c => {
      ...
      if (selectedManagers.length > 0) {
        const addedByUser = team.find(t => t.id === Number(c.addedBy) || t.name.toLowerCase() === (c.recruiterName || "").toLowerCase());
        let candidateManagerId: number | null = null;
        if (addedByUser) {
          if (addedByUser.role === "manager") {
            candidateManagerId = addedByUser.id;
          } else if (addedByUser.role === "tl") {
            candidateManagerId = addedByUser.reportingTo;
          } else if (addedByUser.role === "recruiter") {
            const recruiterReportingTo = team.find(t => t.id === addedByUser.reportingTo);
            if (recruiterReportingTo) {
              if (recruiterReportingTo.role === "manager") {
                candidateManagerId = recruiterReportingTo.id;
              } else if (recruiterReportingTo.role === "tl") {
                candidateManagerId = recruiterReportingTo.reportingTo;
              }
            }
          }
        } else {
          if (c.reportingPerson) {
            const reportingTL = team.find(t => t.role === "tl" && t.name.toLowerCase() === c.reportingPerson.toLowerCase());
            if (reportingTL) {
              candidateManagerId = reportingTL.reportingTo;
            }
          }
        }
        if (!candidateManagerId || !selectedManagers.includes(candidateManagerId)) return false;
      }
  ...
  const managerSummary = useMemo(() => {
    ...
    return managers.map(mgr => {
      const subordinates = team.filter(t => 
        ...
        (t.reportingTo !== null && team.some(tl => tl.role === "tl" && tl.reportingTo === mgr.id && tl.id === t.reportingTo))
      );
      const subIds = subordinates.map(s => s.id);
      const subNames = new Set(subordinates.map(s => s.name.toLowerCase()));

      const mgrCands = candidates.filter(c => {
        const addedBySub = subIds.includes(Number(c.addedBy));
        const nameMatch = c.recruiterName && subNames.has(c.recruiterName.toLowerCase());
        const selfMatch = Number(c.addedBy) === mgr.id;
        return selfMatch || addedBySub || nameMatch;
      });
      ...
      const todayCands = mgrCands.filter(c => new Date(c.createdAt).toDateString() === now.toDateString()).length;
      const weeklyCands = mgrCands.filter(c => new Date(c.createdAt) >= getDaysAgo(7)).length;
      const monthlyCands = mgrCands.filter(c => {
        const date = new Date(c.createdAt);
        return date.getMonth() === now.getMonth() && date.getFullYear() === now.getFullYear();
      }).length;
      ...
      return {
        manager: mgr,
        recruiterCount: subordinates.filter(s => s.role === "recruiter").length,
        tlCount: subordinates.filter(s => s.role === "tl").length,
        total: mgrCands.length,
        todayCands,
        weeklyCands,
        monthlyCands,
        selected,
  ...
  const triggerExport = (format: "csv" | "pdf") => {
    ...
    if (format === "pdf") {
      ...
      const rowsHtml = filteredCandidates.map((c, idx) => `
        <tr style="background: ${idx % 2 === 0 ? "#f8fafc" : "#ffffff"}; border-bottom: 1px solid #cbd5e1;">
          ...
          <td style="padding: 8px;">${c.reportingPerson || "Direct"}</td>
      ...
      printWindow.document.write(`
        ...
        <body>
          ...
          <p>Generated: ${new Date().toLocaleString()} | Boss Executive Data Scope | Filtered Match Size: ${filteredCandidates.length}</p>
    ...
    link.href = url;
    link.setAttribute("download", `Boss_Candidate_Base_Export_${new Date().toISOString().split("T")[0]}.csv`);
  ...
  if (loading) {
    return (
      <div style={{ display: "flex", justifyContent: "center", alignItems: "center", height: "calc(100vh - 120px)", background: "#f8fafc", flexDirection: "column", gap: "15px" }}>
        ...
        <span style={{ fontSize: "1.1rem", fontWeight: 800, color: "#64748b" }}>Loading Candidate's Data...</span>
      </div>
    );
  }
    ...
    <div style={{ padding: "1.5rem", background: "#f8fafc", minHeight: "100%", fontFamily: "'Outfit', 'Inter', sans-serif" }}>
      ...
      <div style={{ background: "white", padding: "1.25rem", borderRadius: "16px", border: "1px solid #e2e8f0", boxShadow: "0 4px 6px -1px rgba(0,0,0,0.02)", marginBottom: "1.5rem" }}>
        ...
        <div style={{ display: "flex", gap: "10px", alignItems: "center", flexWrap: "wrap", marginBottom: "1rem" }}>
          ...
          <div style={{ flex: 1, minWidth: "260px", position: "relative" }}>
            ...
            <input 
              type="text" 
              placeholder="Search candidate name, mobile, email, client, job designation, recruiter, TL, vendor, sourcing..." 
              value={searchQuery}
        ...
        {/* ADVANCED HIERARCHY FILTER DRAWERS */}
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(180px, 1fr))", gap: "12px", borderTop: "1px solid #f1f5f9", paddingTop: "1rem" }}>
          ...
          {/* Manager multiselect checkboxes */}
          <div style={{ display: "flex", flexDirection: "column", gap: "4px" }}>
            <span style={{ fontSize: "0.7rem", color: "#475569", fontWeight: 800, textTransform: "uppercase" }}>Manager-wise filters</span>
            <div style={{ maxHeight: "95px", overflowY: "auto", border: "1px solid #e2e8f0", borderRadius: "8px", padding: "6px", background: "#fafafa" }}>
              {filterLists.managers.map(m => (
                <label key={m.id} style={{ display: "flex", alignItems: "center", gap: "6px", fontSize: "0.78rem", padding: "2px 0", cursor: "pointer", fontWeight: 500 }}>
                  <input type="checkbox" checked={selectedManagers.includes(m.id)} onChange={() => handleToggleManager(m.id)} style={{ accentColor: "#10b981" }} />
                  {m.name}
                </label>
              ))}
            </div>
      ...
      <div style={{ display: "flex", borderBottom: "2px solid #cbd5e1", marginBottom: "1.5rem", gap: "16px", flexWrap: "wrap" }}>
        {[
          ...
          { id: "manager", label: "Manager-Wise View", icon: <LucideBuilding2 size={16} /> },
            ...
            <table style={{ width: "100%", borderCollapse: "collapse", textAlign: "left", fontSize: "0.8rem" }}>
              ...
              <tbody>
                ...
                {filteredCandidates.length === 0 && (
                  <tr>
                    <td colSpan={12} style={{ textAlign: "center", padding: "3rem", color: "#94a3b8", fontWeight: 600 }}>No candidates matched parameters.</td>
                  </tr>
                )}
              </tbody>
            </table>
          ...
          {managerSummary.map(stats => (
            ...
            >
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "8px" }}>
                ...
                <span style={{ background: "#d1fae5", color: "#065f46", padding: "2px 8px", borderRadius: "6px", fontSize: "0.68rem", fontWeight: 800 }}>Manager</span>
              </div>
              <p style={{ color: "#64748b", fontSize: "0.78rem", margin: "0 0 12px" }}>Reporters: {stats.tlCount} TLs • {stats.recruiterCount} Recruiters</p>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(280px, 1fr))", gap: "16px" }}>
          {tlSummary.map(stats => (
            ...
            >
              ...
              <p style={{ color: "#64748b", fontSize: "0.78rem", margin: "0 0 12px" }}>Reporters: {stats.recruiterCount} Recruiters</p>
          ...
          <div style={{ overflowX: "auto" }}>
            <table style={{ width: "100%", borderCollapse: "collapse", textAlign: "left", fontSize: "0.8rem" }}>
              ...
              <tbody>
                ...
                {filteredCandidates.length === 0 && (
                  <tr>
                    <td colSpan={7} style={{ textAlign: "center", padding: "3rem", color: "#94a3b8" }}>Select recruiters or TLs above to load sandbox pool.</td>
                  </tr>
                )}
              </tbody>
            </table>
          ...
          <div style={{ position: "fixed", inset: 0, zIndex: 9999, background: "rgba(15, 23, 42, 0.4)", backdropFilter: "blur(2px)", display: "flex", justifyContent: "flex-end" }}>
            ...
            >
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", borderBottom: "1px solid #e2e8f0", paddingBottom: "12px", marginBottom: "1rem" }}>
                <h3 style={{ fontSize: "1.1rem", fontWeight: 900, color: "#0f172a", margin: 0 }}>Manager Candidate Base</h3>
              ...
              {(() => {
                const mgrStats = managerSummary.find(s => s.manager.id === selectedManagerProfile.id);
                if (!mgrStats) return null;

                const subordinates = team.filter(t => 
                  t.reportingTo === selectedManagerProfile.id || 
                  (t.reportingTo !== null && team.some(tl => tl.role === "tl" && tl.reportingTo === selectedManagerProfile.id && tl.id === t.reportingTo))
                );
                const subIds = subordinates.map(s => s.id);
                const subNames = new Set(subordinates.map(s => s.name.toLowerCase()));
                ...
                const mgrCands = candidates.filter(c => {
                  const addedBySub = subIds.includes(Number(c.addedBy));
                  const nameMatch = c.recruiterName && subNames.has(c.recruiterName.toLowerCase());
                  const selfMatch = Number(c.addedBy) === selectedManagerProfile.id;
                  return selfMatch || addedBySub || nameMatch;
                });

                const tlsList = subordinates.filter(s => s.role === "tl");
                const contributionList = tlsList.map(tl => {
                  const tlSubs = team.filter(t => t.reportingTo === tl.id);
                  const tlSubIds = tlSubs.map(s => s.id);
                  const tlSubNames = new Set(tlSubs.map(s => s.name.toLowerCase()));

                  const tlCands = mgrCands.filter(c => {
                    const reportingTLMatch = c.reportingPerson && c.reportingPerson.toLowerCase() === tl.name.toLowerCase();
                    const addedBySub = tlSubIds.includes(Number(c.addedBy));
                    const nameMatch = c.recruiterName && tlSubNames.has(c.recruiterName.toLowerCase());
                    return reportingTLMatch || addedBySub || nameMatch;
                  });
                ...
                return (
                  <div style={{ display: "flex", flexDirection: "column", gap: "1.25rem" }}>
                    ...
                    <div style={{ background: "linear-gradient(135deg, #d1fae5 0%, #ecfdf5 100%)", padding: "1rem", borderRadius: "14px", border: "1px solid #6ee7b7" }}>
                      <h4 style={{ margin: "0 0 2px", fontSize: "1.1rem", fontWeight: 900 }}>{selectedManagerProfile.name}</h4>
                      <p style={{ color: "#065f46", fontSize: "0.78rem", margin: "0 0 10px" }}>Hierarchy Level: Manager Node</p>
                      ...
                      <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: "6px", fontSize: "0.75rem", borderTop: "1px dashed #6ee7b7", paddingTop: "8px" }}>
                        <div>TLs: <strong>{mgrStats.tlCount}</strong></div>
                        <div>Recruiters: <strong>{mgrStats.recruiterCount}</strong></div>
                        <div>Lifetime: <strong>{mgrStats.total}</strong></div>
                      </div>
                    </div>

                    <div>
                      <h5 style={{ fontSize: "0.75rem", fontWeight: 800, color: "#475569", textTransform: "uppercase", marginBottom: "6px" }}>TL contribution roster</h5>
                ...
                return (
                  <div style={{ display: "flex", flexDirection: "column", gap: "1rem" }}>
                    ...
                    <div style={{ background: "#f8fafc", padding: "1rem", borderRadius: "14px", border: "1px solid #cbd5e1" }}>
                      ...
                      <p style={{ color: "#64748b", fontSize: "0.78rem", margin: "0 0 10px" }}>Reporting TL: <strong>{selectedRecruiterProfile.manager_tl?.name || "Direct"}</strong></p>
            ...
            >
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: "14px" }}>
                ...
                <div style={{ border: "1px solid #cbd5e1", borderRadius: "12px", padding: "10px", background: "#f8fafc", display: "grid", gridTemplateColumns: "1fr 1fr", gap: "10px", fontSize: "0.78rem" }}>
                  ...
                  <div>TL Oversight: <strong>{selectedCandidate.reportingPerson || "Direct"}</strong></div>
                ...
                <div style={{ border: "1px solid #cbd5e1", borderRadius: "12px", padding: "10px" }}>
                  ...
                  {selectedCandidate.interviewDate ? (
                    ...
                    <div style={{ fontSize: "0.75rem", color: "#94a3b8" }}>No interview scheduled.</div>
                  )}
                </div>
