...
export default function ReportingAI({ currentUser, candidates = [], onRefresh }: ReportingAIProps) {
  ...
  const getVendorNameOuter = (c: any) => {
    try {
  ...
  const getSeniorNameOuter = (c: any) => {
    if (currentUser?.manager_tl && currentUser.manager_tl.name) {
      return currentUser.manager_tl.name;
    }
    if (currentUser?.manager && currentUser.manager.name) {
      return currentUser.manager.name;
    }
    return "N/A";
  };
    ...
    <div className="module-container" style={{ position: "relative", height: "100%", overflowY: "auto", background: "#f8fafc", padding: "1.25rem", fontFamily: "'Outfit', 'Inter', sans-serif" }}>
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "1.25rem", flexWrap: "wrap", gap: "10px" }}>
        ...
        <div style={{ display: "flex", gap: "10px" }}>
          ...
          >
            <LucideFileText size={14} /> View Lineup Sheet
          </button>
          ...
          <div style={{ position: "fixed", inset: 0, zIndex: 99999, background: "rgba(15, 23, 42, 0.5)", backdropFilter: "blur(4px)", display: "flex", justifyContent: "center", alignItems: "center" }}>
            ...
            >
              ...
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "1.5rem", borderBottom: "1px solid #f1f5f9", paddingBottom: "12px" }}>
                <div>
                  <h3 style={{ fontSize: "1.25rem", fontWeight: 800, color: "#0f172a", margin: 0, display: "flex", alignItems: "center", gap: "8px" }}>
                    <LucideFileText size={20} color="#2563eb" /> Lineup Sheet
                  </h3>
                  <p style={{ color: "#64748b", fontSize: "0.82rem", margin: "4px 0 0 0" }}>
                    Showing {filterCandidatesByPeriod().length} candidate records.
                  </p>
                </div>
                <button
              ...
              <div className="custom-scrollbar" style={{ flex: 1, overflowY: "auto", overflowX: "auto", border: "1px solid #e2e8f0", borderRadius: "12px", background: "#f8fafc" }}>
                {filterCandidatesByPeriod().length === 0 ? (
                  <div style={{ padding: "4rem 2rem", textAlign: "center", color: "#94a3b8" }}>
                    ...
                    <p style={{ fontSize: "0.8rem", margin: "4px 0 0 0" }}>No candidates sourced in the selected date period.</p>
                  </div>
                ) : (
                  <table style={{ width: "100%", borderCollapse: "collapse", textAlign: "left", fontSize: "0.8rem" }}>
                    ...
                    <tbody>
                      {filterCandidatesByPeriod().map((c, idx) => {
                        const sourcingDateStr = c.sourcingDate || (c.createdAt ? String(c.createdAt).split("T")[0] : "N/A");
                        const remarkStatusStr = c.remarks || c.status || c.cvStatus || "N/A";
                        const hasJoined = remarkStatusStr.toLowerCase().includes("joined") || remarkStatusStr.toLowerCase().includes("hired");
                        ...
                        return (
                          ...
                          >
                            ...
                            <td style={{ padding: "12px 16px", color: "#475569", whiteSpace: "nowrap" }}>{currentUser?.name || "N/A"}</td>
                            <td style={{ padding: "12px 16px", color: "#475569", whiteSpace: "nowrap" }}>{getVendorNameOuter(c)}</td>
                            ...
                            <td style={{ padding: "12px 16px", color: "#475569", fontWeight: 600, whiteSpace: "nowrap" }}>{getSeniorNameOuter(c)}</td>
                          </tr>
                        );
                      })}
                    </tbody>
              ...
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginTop: "1.5rem", borderTop: "1px solid #f1f5f9", paddingTop: "12px", flexWrap: "wrap", gap: "10px" }}>
                <span style={{ fontSize: "0.8rem", color: "#64748b", fontWeight: 600 }}>
                  Active Audit Filter Summary: <strong style={{ color: "#0f172a" }}>{dateFilter.toUpperCase()}</strong>
                </span>
                <button
                  onClick={() => setShowLineupSheet(false)}
                  style={{
