...
import MeetingRecruiterTab from "@/components/dashboard/MeetingRecruiterTab";
...
export const ReminderSystem = ({ 
  ...
  useEffect(() => {
    ...
    if (active && (!active.designation || active.designation === "N/A")) {
      ...
      if (fullCandidate) {
        ...
        safeSetItem(storeKey, JSON.stringify(updated));
      }
    }
  ...
  useEffect(() => {
    const checkInterval = setInterval(() => {
      ...
      if (!currentActiveId) {
        ...
        if (nextReady) {
          safeSetItem(activeKey, nextReady.id);
          setActive(nextReady);
        }
      }

      if (updated) {
        safeSetItem(storeKey, JSON.stringify(savedReminders));
      }
    }, 3000);
  ...
  const removeReminder = (id: string) => {
    ...
    safeSetItem(storeKey, JSON.stringify(filtered));
    localStorage.removeItem(activeKey);
    setActive(null);
  };
  ...
  const handleOutcomeAction = async (newStatus: string) => {
    ...
    if (newStatus === "Selected") {
      ...
      updateReminder(active.id, { type: 'JOINING_CHECK', triggerAt: Date.now() + 5 * 60 * 60 * 1000, remarks: newStatus });
...
export default function RecruiterDashboard() {
  ...
  const renderContent = () => {
    switch (tab) {
      ...
      case "meetings": return <MeetingRecruiterTab currentUser={userProfile} role="recruiter" />;
...
const DashboardHome = ({ 
  ...
  const handleMarkTodoComplete = (todoId: string) => {
    ...
    setTodos(updated);
    safeSetItem("givyansh_todo_tasks_v1", JSON.stringify(updated));
...
export const CRMView = ({ candidates, loading, selectedCandidate, setSelectedCandidate, onRefresh, type = "crm", currentUser }: any) => {
  ...
  useEffect(() => {
    ...
    if (viewState === "form" && !formData.id && !formData._id) {
      safeSetItem(draftKey, JSON.stringify(formData));
    }
    ...
    const loadVendors = async () => {
      ...
      try {
        ...
        if (res.ok) {
          const data = await res.json();
          if (Array.isArray(data)) {
            dbVendors = data.filter(Boolean).map((v: any) => ({
              id: String(v.id || ""),
              name: v.name || "",
              company: v.company || "",
      ...
      if (stored) {
        try {
          ...
          if (Array.isArray(parsed)) {
            localVendors = parsed.filter(Boolean).map((v: any) => ({
              id: String(v.id || ""),
              name: v.name || "",
              company: v.company || "",
    ...
    try {
      ...
      if (res.ok) {
        ...
        if (candId) {
          ...
          safeSetItem(key, JSON.stringify(mappings));
        }
        const draftKey = `fast_rms_new_candidate_draft_${currentUser?.id || 'default'}`;
        ...
        <div className="registration-form-container" style={{ padding: "2rem 2.5rem" }}>
          ...
          <div className="form-grid-3">
            ...
            <div className="input-group" id="field-clientName">
              ...
              <SearchableSelect options={clients.filter((c: any) => c && (!c.isHold || c.name === formData.clientName)).map((c: any) => c?.name || "")} value={formData.clientName} onChange={v => handleFieldChange("clientName", v)} placeholder="Select Client" />
            ...
            <div className="input-group" id="field-vendorId">
              ...
              <SearchableSelect 
                options={[...vendors.filter(Boolean).map((v: any) => `${v.name} (${v.company})`), "None"]} 
                value={formData.vendorId ? (formData.vendorId === "none" ? "None" : (() => {
                  const found = vendors.find((v: any) => v && String(v.id) === String(formData.vendorId));
                  return found ? `${found.name} (${found.company})` : "";
                })()) : ""} 
                onChange={(v) => {
                  if (v === "None") {
                    setFormData((prev: any) => ({ ...prev, vendorId: "none" }));
                  } else {
                    const found = vendors.find((vend: any) => vend && `${vend.name} (${vend.company})` === v);
                    if (found) {
  ...
  const todayStr = new Date().toISOString().split('T')[0];
  const uniqueDesignations = Array.from(new Set(jobs.filter(Boolean).map((j: any) => j.title))).filter(Boolean).sort();
      ...
      <div className="full-pool-container">
        <div className="crm-dash-header" style={{ borderBottom: "1px solid #f1f5f9", flexShrink: 0, padding: "1.25rem 2rem 1.25rem 1rem", display: "flex", alignItems: "center", justifyContent: "space-between", gap: "2rem", background: "white" }}>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: "0.6rem", flexShrink: 0 }}>
             ...
             {currentUser?.email === "givyansh779081@gmail.com" && (
                <>
                  <button
               ...
               >
                  <div className="filter-inner-grid" style={{ padding: "1.5rem 2.5rem", display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(200px, 1fr))", gap: "1rem" }}>
                     ...
                     <div className="filter-group">
                        ...
                        <SearchableSelect 
                           options={["All Clients", ...clients.map((cl: any) => cl.name)]} 
                           value={activeFilters.clientName ? activeFilters.clientName : "All Clients"} 
                           onChange={v => setActiveFilters({...activeFilters, clientName: v === "All Clients" ? "" : v})} 
                     ...
                     <div className="filter-group">
                        ...
                        <SearchableSelect 
                           options={["All Designations", ...uniqueDesignations as string[]]} 
                           value={activeFilters.designation ? activeFilters.designation : "All Designations"} 
                           onChange={v => setActiveFilters({...activeFilters, designation: v === "All Designations" ? "" : v})} 
                     ...
                     <div className="filter-group">
                        ...
                        <SearchableSelect 
                           options={["All States", "Andhra Pradesh","Arunachal Pradesh","Assam","Bihar","Chhattisgarh","Goa","Gujarat","Haryana","Himachal Pradesh","Jharkhand","Karnataka","Kerala","Madhya Pradesh","Maharashtra","Manipur","Meghalaya","Mizoram","Nagaland","Odisha","Punjab","Rajasthan","Sikkim","Tamil Nadu","Telangana","Tripura","Uttar Pradesh","Uttarakhand","West Bengal","Andaman and Nicobar Islands","Chandigarh","Dadra and Nagar Haveli and Daman and Diu","Delhi","Jammu and Kashmir","Ladakh","Lakshadweep","Puducherry"]} 
                           value={activeFilters.state ? activeFilters.state : "All States"} 
                           onChange={v => setActiveFilters({...activeFilters, state: v === "All States" ? "" : v})} 
                     ...
                     <div className="filter-group">
                        ...
                        <SearchableSelect 
                           ...
                           value={activeFilters.gender ? activeFilters.gender : "All"} 
                           onChange={v => setActiveFilters({...activeFilters, gender: v === "All" ? "" : v})} 
                           placeholder="All" 
                        />
                     </div>
                     <div className="filter-group">
                     ...
                     <div className="filter-group">
                        ...
                        <SearchableSelect 
                           options={["All", "BA","BSc","BCom","BBA","BCA","BTech / BE","MBBS","BDS","BPharma","LLB","BEd","MA","MSc","MCom","MBA","MCA","MTech","LLM","MEd","PhD","CA","CS","CMA","CFA","Below 10th","10th Pass","12th Pass","ITI","Polytechnic Diploma","Other Diploma","Certificate Course","Other"]} 
                           value={activeFilters.qualification ? activeFilters.qualification : "All"} 
                           onChange={v => setActiveFilters({...activeFilters, qualification: v === "All" ? "" : v})} 
                           placeholder="All" 
                        />
                     </div>
                     <div className="filter-group">
                        ...
                        <SearchableSelect 
                           options={["All", "Fresher","Internship","0-6 Months","1 Year","2 Years","3 Years","4 Years","5 Years","6 Years","7 Years","8 Years","9 Years","10 Years","10+ Years","N/A"]} 
                           value={activeFilters.totalExperience ? activeFilters.totalExperience : "All"} 
                           onChange={v => setActiveFilters({...activeFilters, totalExperience: v === "All" ? "" : v})} 
                           placeholder="All" 
                        />
                     </div>
                     <div className="filter-group">
                        ...
                        <SearchableSelect 
                           options={["All", "Shared","Non Shared"]} 
                           value={activeFilters.cvStatus ? activeFilters.cvStatus : "All"} 
                           onChange={v => setActiveFilters({...activeFilters, cvStatus: v === "All" ? "" : v})} 
                           placeholder="All" 
                        />
                     </div>
                     <div className="filter-group">
                        ...
                        <SearchableSelect 
                           options={["All", "Connected","Interested","Not Connected","Not Interested","Joined","Dropped","Selected","Process for joining","Rejected","Interview Done","Interview Not Done","Round 1 Done","Round 2 Done","Round 3 Done","Round 4 Done","Round 5 Done","Processing For Next Round","All Rounds Done"]} 
                           value={activeFilters.remarks ? activeFilters.remarks : "All"} 
                           onChange={v => setActiveFilters({...activeFilters, remarks: v === "All" ? "" : v})} 
                           placeholder="All" 
                        />
                     </div>
                     <div className="filter-group">
                        ...
                        <SearchableSelect 
                           options={["All Platforms", ...sourcingPlatforms.map((p: any) => p.name)]} 
                           value={activeFilters.sourcingBy ? activeFilters.sourcingBy : "All Platforms"} 
                           onChange={v => setActiveFilters({...activeFilters, sourcingBy: v === "All Platforms" ? "" : v})} 
                     ...
                     <div className="filter-group">
                        ...
                        <SearchableSelect 
                           options={["All Vendors", ...vendors.map((v: any) => `${v.name} (${v.company})`)]} 
                           value={activeFilters.vendorId ? (vendors.find((v: any) => String(v.id) === String(activeFilters.vendorId)) ? `${vendors.find((v: any) => String(v.id) === String(activeFilters.vendorId)).name} (${vendors.find((v: any) => String(v.id) === String(activeFilters.vendorId)).company})` : "") : "All Vendors"} 
                           onChange={v => {
                             if (v === "All Vendors") {
                               setActiveFilters({ ...activeFilters, vendorId: "" });
                             } else {
                  ...
                  <tbody>
                     {filteredCandidates.map((c: any, idx: number) => (
                       ...
                       >
                          ...
                          <td>
                             <span className={`status-pill ${(c.remarks || "New").toLowerCase().replace(" ", "-")}`}>
                                {c.remarks || "New"}
                             </span>
                          </td>
             ...
             <div style={{ marginTop: "1.5rem", paddingTop: "1rem", borderTop: "1px solid #e2e8f0", display: "flex", justifyContent: "flex-end", gap: "10px" }}>
                ...
                <button 
                  onClick={() => {
                     ...
                     safeSetItem("givyansh_lead_data_v1", JSON.stringify(d));
                     ...
                     fetch(`/api/candidates/${candidateId}`, {
                       ...
                       body: JSON.stringify({
                         dataType: "lead",
                         sector: finalCategories[0] || "",
                         remarks: leadDataForm.remarks
                       })
                     })
                     .then(res => {
                       if (res.ok && onRefresh) {
                     ...
                     .catch(err => console.error("Failed to sync candidate lead status to database:", err));
